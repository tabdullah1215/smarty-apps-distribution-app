    ...
    try {
        ...
        switch (action) {
            ...
            case 'getDistributors':
                return await handleFetchPendingDistributors();
...
async function handleFetchPendingDistributors() {
    try {
        console.log('Fetching pending distributors');
        const scanResult = await ddbDocClient.send(new ScanCommand({
            TableName: 'Distributors',
        ...
        console.log('Pending distributors fetched:', scanResult.Items.length);
        return createResponse(200, scanResult.Items);
    } catch (error) {
        console.error('Error fetching pending distributors:', error);
        return createResponse(500, { message: 'Error fetching pending distributors', error: error.message });
    }
}
